...
namespace cryptonote
{
  ...
  enum quenero_version
  {
    quenero_version_0 = 0,
    quenero_version_1,
    quenero_version_2,
    quenero_version_3_per_output_unlock_times,
    quenero_version_4_tx_types,
  };
  ...
  public:
    ...
    //
    // NOTE: Quenero specific
    //
    std::vector<uint64_t> output_unlock_times;
    enum quenero_type_t
    {
      quenero_type_standard,
      quenero_type_deregister,
      quenero_type_key_image_unlock,
      quenero_type_count,
    };
    ...
    BEGIN_SERIALIZE()
      ...
      if (version > quenero_version_2 && (blob_type == BLOB_TYPE_CRYPTONOTE_QUENERO || blob_type == BLOB_TYPE_CRYPTONOTE_XTNC))
      {
        FIELD(output_unlock_times)
        if (version == quenero_version_3_per_output_unlock_times)
          FIELD(is_deregister)
      }
      VARINT_FIELD(unlock_time)
      FIELD(vin)
      FIELD(vout)
      if (blob_type == BLOB_TYPE_CRYPTONOTE_QUENERO || blob_type == BLOB_TYPE_CRYPTONOTE_XTNC)
      {
        if (version >= quenero_version_3_per_output_unlock_times && vout.size() != output_unlock_times.size()) return false;
      }
      FIELD(extra)
      if ((blob_type == BLOB_TYPE_CRYPTONOTE_QUENERO || blob_type == BLOB_TYPE_CRYPTONOTE_XTNC) && version >= quenero_version_4_tx_types)
      {
        VARINT_FIELD(type)
        if (static_cast<uint16_t>(type) >= quenero_type_count) return false;
      }
